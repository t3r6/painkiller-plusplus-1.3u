    ...
    if pks and pks.Bot and pks._Entity ~= nil then
      ...
      if Game.PlayerStats[botclientid] ~= nil then
        ...
        if self.bot[botclientid].state == BotStates.SearchingForWaypoint then
          ...
          elseif Cfg.BotChat and math.floor(math.random(5000)) == 1 or Cfg.BotChat and Cfg.BotEliza and Game.PlayerStats[botclientid].LastThingHeard ~= nil then
            self.bot[botclientid].state = BotStates.Typing
            self.bot[botclientid].statetime = INP.GetTime() + 2
          else
            self.bot[botclientid].angle = self.bot[botclientid].angle + 0.001 * self.bot[botclientid].rotation
        ...
        elseif self.bot[botclientid].state == BotStates.HeadingToWaypoint then
          ...
          self.bot[botclientid].angle = self.bot[botclientid].angle - (self.bot[botclientid].angle - angle) * 0.2 + math.pi / 200 * math.sin(INP.GetTime())
        ...
        for i, o in Game.Players, nil do
          if o.ClientID == botclientid then
            ...
            if self.bot[botclientid].state == BotStates.HeadingToWaypoint or self.bot[botclientid].state == BotStates.AttackingEnemy and 80 < arm or self.bot[botclientid].state == BotStates.AttackingEnemy and self.bot[botclientid].weapon == 1 then
              if o:IsOnGround() then
                ...
                elseif arm < 8 and 0 <= arm and self.bot[botclientid].state == BotStates.AttackingEnemy and self.bot[botclientid].weapon ~= 1 then
...
function Game:GetAttackTarget(botclientid)
  ...
  for i, ps in Game.PlayerStats, nil do
    if ps and ps ~= nil and ps._Entity ~= nil and ps.ClientID ~= botclientid and ps.Spectator == 0 and (not ((not (Game.PlayerStats[botclientid].Team ~= ps.Team and MPGameRules[MPCfg.GameMode]) or not MPGameRules[MPCfg.GameMode].Teams) and MPGameRules[MPCfg.GameMode]) or not MPGameRules[MPCfg.GameMode].Teams) then
      ...
      if min > distance and distance < 80 then
        ...
        if not b then
          ...
          elseif wep == 4 and alt == false or wep == 5 and alt == false or wep == 1 and alt == true or wep == 2 and alt == true then
            nearestx = mex + vx * time
            nearestz = mez + vz * time
            if wep == 4 and alt == false then
...
function Game:CheckBotCount()
  if Game:IsServer() then
    ...
    if Cfg.BotMinPlayers == maxplayers + 1 and playercount < maxplayers and playercount < Cfg.BotMinPlayers and speccount == 0 then
      Console:Cmd_ADDBOT()
      return
    end
