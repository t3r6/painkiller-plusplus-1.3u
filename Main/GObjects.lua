...
function GObjects:AddToTables(obj)
  ...
  if obj.Tick2 then
    AddObjToTable(self.Tick2List, obj)
  end
  if obj.Tick3 then
    AddObjToTable(self.Tick3List, obj)
  end
  if obj.Render then
    AddObjToTable(self.RenderList, obj)
  end
  if obj.PostRender then
    AddObjToTable(self.PostRenderList, obj)
  end
  if obj.Synchronize then
    AddObjToTable(self.SynchronizeList, obj)
  end
  if obj._Timers and table.getn(obj._Timers) > 0 then
    AddObjToTable(self.TimerList, obj)
  end
end

function GObjects:Apply()
  PMENU.SetLoadingScreenOverall(PMENU.GetLoadingScreenOverall() + table.getn(self.Elements), 20)
  for i, o in self.Elements, nil do
...
function GObjects:Update()
  ...
  luaProfiler_LOGIC3a()
  for i, v in self.UpdateListActors, nil do
...
function GObjects:Tick(delta)
  ...
  function func(i, v)
    RemoveObjFromTable(self.TimerList, v)
  end
...
function GObjects:Synchronize(delta)
  Lev:Synchronize()
  table.foreachi(self.SynchronizeList, function(i, v)
    v:Synchronize()
  end)
end

function GObjects:Render(delta)
  table.foreachi(self.RenderList, function(i, v)
    v:Render(delta)
  end)
end

function GObjects:PostRender(delta)
  table.foreachi(self.PostRenderList, function(i, v)
    v:PostRender(delta)
  end)
end
...
function GObjects:Tick2(delta)
  ...
  table.foreachi(self.Tick2List, function(i, v)
    v:Tick2(delta)
  end)
end
...
function GObjects:Tick3(delta)
  ...
  table.foreachi(self.Tick3List, function(i, v)
    v:Tick3(delta)
  end)
end

function GObjects:EditRender(delta)
  if Lev.EditRender then
    Lev:EditRender()
  end
  for i, o in self.Elements, nil do
    if o.EditRender then
      o:EditRender(delta)
    end
  end
end
...
function GObjects:EditTick(delta)
  for i, o in self.Elements, nil do
    ...
    if o.Synchronize then
      o:Synchronize()
    end
    if o._Synchronize then
      o:_Synchronize()
    end
  end
end
...
function GObjects:Clear()
  for i, o in self.Elements, nil do
    if not o._ToKill and o.OnToKill then
      o:OnToKill()
    end
    if o.Delete then
      o:Delete()
...
function GObjects:Delete(obj)
  if obj == Editor.SelObj then
    Editor.SelObj = nil
  end
  if not obj._ToKill and obj.OnToKill then
    obj:OnToKill()
  end
  if obj.Delete then
    obj:Delete()
  ...
  if obj.BaseObj == "CheckPoint.CItem" or obj.BaseObj == "EndOfGame.CItem" or obj.BaseObj == "EndOfLevel.CItem" or obj.BaseObj == "pentakl.CItem" then
    for i, v in self.CheckPoints, nil do
      if v == obj then
  ...
  if obj.Render then
    RemoveObjFromTable(self.RenderList, obj)
  end
  if obj.PostRender then
    RemoveObjFromTable(self.PostRenderList, obj)
  end
  if obj.Tick2 then
    RemoveObjFromTable(self.Tick2List, obj)
  end
  if obj.Tick3 then
    RemoveObjFromTable(self.Tick3List, obj)
  end
  if obj.Synchronize then
    RemoveObjFromTable(self.SynchronizeList, obj)
  end
...
function GObjects:Rescale(factor, class)
  ...
  for i, o in self.Elements, nil do
    if (not class or class == o._Class) and o.Rescale then
      o:Rescale(factor)
    end
  end
end
...
function GObjects:ToKill(obj, no_entity)
  ...
  if no_entity then
    obj._NoEntity = true
  elseif Game.GMode == GModes.SingleGame then
    ENTITY.EnableCollisions(obj._Entity, false)
  end
  obj._ToKill = true
  if obj.OnToKill then
    obj:OnToKill()
  end
  if obj.Actions then
