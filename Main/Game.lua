...
function Game:Init(nolevel)
  ...
  table.foreachi(SoundsProperties, function(i, v)
...
function Game:Tick(delta)
  ...
  elseif self.GoldenEnabled == false then
    ...
    if INP.Action(Actions.UseCards) and 0 < self.Difficulty and self.IsDemon == false and self.GoldenEnabledStart == 0 then
...
function Game:Update()
  ...
  elseif Game.FlashDemon and Player and Player.SoulsCount >= Game.Demon_Counter - 2 and Player.SoulsCount < Game.Demon_Counter then
...
function Game:IsFxObject(ext)
  if ext == "CBillboard" or ext == "CLight" or ext == "CParticleFX" or ext == "CEnvironment" or ext == "CSound" or ext == "CAcousticEnv" then
    return true
  end
  return false
end
...
function Game:CountObjectsDirectory(path)
  ...
  for i = 1, table.getn(files) do
    ...
    if ext ~= "CLevel" then
      local fx = self:IsFxObject(ext)
      if Game.GMode == GModes.SingleGame or Game.GMode == GModes.MultiplayerServer or Game.GMode == GModes.MultiplayerClient and fx or Game.GMode == GModes.DedicatedServer and not fx then
        count = count + 1
      end
    end
  end
...
function Game_GetMsg(msg, ...)
  ...
  if msg == "EXPLOSION" then
    ...
    if obj and not obj._ToKill and obj.Health and (not obj._Exploded or obj._Exploded ~= arg[5]) then
      obj._Exploded = arg[5]
      local damage = arg[8]
      if 0 < damage and obj.OnDamage and (not obj._GotInstantExplosion or Game.Counter ~= obj._GotInstantExplosion) then
        local killer
        if arg[6] then
          if Game.GMode == GModes.SingleGame then
...
function Game:OnPlay(firstTime)
  ...
  if firstTime then
    ...
    table.foreachi(GObjects.Elements, function(i, v)
...
function Game:CountMoneyAndActorsOnLevel()
  ...
  for i, v in GObjects.Elements, nil do
    if v._Class == "CSpawnPoint" then
      ...
      elseif not Templates[v.SpawnTemplate].NotCountable and not v.NotCountable and Templates[v.SpawnTemplate]._Class == "CActor" then
