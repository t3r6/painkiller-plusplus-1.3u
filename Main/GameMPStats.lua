function Game:UpdateSpecs()
  if Game:IsServer() then
    for i, o in Game.PlayerStats, nil do
      if Game.PlayerStats[o.ClientID].Version then
    ...
    for id, ps in Game.PlayerStats, nil do
      if ps and ps.Player ~= nil and ps.Spectator == 0 then
        ...
        if ps.ClientID ~= nil and ps.Player.Health ~= nil and ps.Player.Armor ~= nil and currentweapon ~= nil and ammo1 ~= nil and ammo2 ~= nil then
          Game.PlayerData[ps.ClientID] = {
            Health = ps.Player.Health,
            Armour = ps.Player.Armor,
  ...
  if Game:IsServer() then
    for i, o in Game.PlayerStats, nil do
      if o.ClientID == clientid then
        for ig, ps in Game.PlayerStats, nil do
          if Game.Stats[ps.ClientID] ~= nil then
            for id, attack_type in Game.Stats[ps.ClientID].PlayerWepStats, nil do
...
function Game:SetStats(clientid, attacktype, deltahit, deltashot, deltadamage)
  ...
  if deltahit == nil then
    deltahit = 0
  end
  if deltashot == nil then
    deltashot = 0
  end
  if deltadamage == nil then
    deltadamage = 0
  end
  if Game.Stats[clientid] == nil then
    Game.Stats[clientid] = {
      PlayerWepStats = {}
    }
  end
  if Game.Stats[clientid].PlayerWepStats[attacktype] == nil then
    Game.Stats[clientid].PlayerWepStats[attacktype] = {}
...
function Game:AddToStats(clientid, attacktype, deltahit, deltashot, deltadamage)
  if not clientid or clientid == nil or clientid < 0 then
    return
  end
  if attacktype == nil then
    return
