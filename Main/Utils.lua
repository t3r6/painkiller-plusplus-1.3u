...
function InheritFunctionsAndStatics(obj, t)
  ...
  while i do
    ...
    elseif obj[i] and type(obj[i]) == "table" and type(v) == "table" then
      InheritFunctionsAndStatics(obj[i], v)
    end
    i, v = next(t, i)
  end
end
...
function SaveTable(f, tab, name, diff, parent)
  ...
  while i do
    if not getmetatable(v) and string.sub(i, 1, 1) ~= "_" and string.sub(i, 1, 2) ~= "s_" and type(v) ~= "function" then
        ...
        if string.sub(i, 1, 3) ~= "Obj" and string.sub(i, 1, 4) ~= "_Obj" and string.sub(i, 1, 2) ~= "r_" and string.sub(i, 1, 3) ~= "_r_" then
          ...
          if type(i) == "number" then
            if v.SaveString then
              if not (diff and np) or not v:Compare(np) then
                f:write(name .. "[" .. i .. "] = " .. v:SaveString() .. "\n")
              end
            else
              if not diff or not np then
                f:write(name .. "[" .. i .. "] = " .. typ .. "\n")
              end
              SaveTable(f, v, name .. "[" .. i .. "]", diff, np)
            end
          elseif v.SaveString then
            if not (diff and np) or not v:Compare(np) then
              f:write(name .. "." .. i .. " = " .. v:SaveString() .. "\n")
            end
          else
      ...
      elseif not diff or np == nil or np ~= v then
        local val = v
  ...
  for i, v in tab, nil do
    ...
    if (itype == "number" or itype == "string" and string.sub(i, 1, 2) ~= "s_") and vtype ~= "function" and not getmetatable(v) then
      if vtype == "table" then
        ...
        if vtype == "boolean" and v == true then
          val = "true"
        end
        if vtype == "boolean" and v == false then
          val = "false"
        end
...
function Chance(var)
  if 1 <= var and math.random(var) == math.floor(var) then
    return true
  end
  return false
end
...
function CloneTemplate(template)
  ...
  if i then
    template = string.sub(template, 1, i - 1) .. "_" .. string.sub(template, i + 1)
  end
  ...
  if not obj then
    MsgBox("Template not found :" .. template)
  end
  if obj.OnInitTemplate then
    obj:OnInitTemplate()
  end
  return obj
end
...
function PreloadTemplates(path, leveltemplates)
  ...
  local dirs = FS.FindFiles(path .. "/*.*", 0, 1)
  for i = 1, table.getn(dirs) do
    if Game.GMode == GModes.SingleGame or Game.GMode ~= GModes.SingleGame and not leveltemplates and dirs[i] ~= "Monsters" and dirs[i] ~= "Items" and dirs[i] ~= "Treasures" then
...
function CloneObj(obj, inherit)
  local baseobj, objname
  if type(obj) == "string" then
    baseobj = obj
    obj = FindObj(baseobj)
  elseif obj._Name ~= "NoName" then
    baseobj = obj._Name
  else
    baseobj = obj._Class
  end
  ...
  if newobj.OnClone then
    newobj:OnClone(obj)
  end
  if inherit then
    newobj.BaseObj = baseobj
  end
  return newobj
end

function SaveObj(filename, obj)
  if obj.NotSaveable then
    return
  end
...
BaseObj:  ']] .. newobj.BaseObj .. "' not found!!!")
  ...
  getfenv()[name] = newobj
  if templatemode and newobj.s_SubClass then
    newobj.s_SubClass = PureClone(newobj.s_SubClass)
  end
  o = newobj
  if XBOX then
    mt = getmetatable(o)
...
function CompareObjToBaseObjVar(obj, basename, fullkey)
  ...
  for i = 1, n do
    ...
    if type(obj) ~= "table" and bo ~= obj then
      return false
    end
  end
  return true
...
function BindFX(entity, name, scale, joint, ox, oy, oz, cox, coy, coz, ax, ay, az)
  ...
  if type(joint) == "string" then
    joint = MDL.GetJointIndex(entity, joint)
  end
  if ox then
...
function BindSoundToEntity(entity, soundname, fstart, fend, looped, joint, ox, oy, oz, delay, velFactor, dontautodelete)
  ...
  if ox then
    ENTITY.SetPosition(snd, ox, oy, oz)
  end
  ...
  SND.Play(snd, delay)
  if velFactor then
    SND.SetVelocityScaleFactor(snd, velFactor)
  end
  return snd, idx
end
...
function CanBurning(obj)
  return obj and obj.OnDamage and not obj.notBleeding and not obj.DisableFreeze and (obj._Class ~= "CItem" or not not obj.DestroyPack)
end

function BurnObject(obj)
  if CanBurning(obj) and not obj._burning then
